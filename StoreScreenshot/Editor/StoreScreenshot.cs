...
public class StoreScreenshot : EditorWindow
{
    ...
    [MenuItem("Tools/Store Screenshot/Settings")]
    static void Init()
    {
        ReadValues();
        ...
        resolutions.Clear();
        resolutions.Add(new StoreScreenshotUtils.ScreenshotResolution()
        {
            resName = "Icon",
            width = 512,
            height = 512,
    ...
    static void SaveResolutions()
    {
        ...
        {
            if (resolutionsToSave[i].width != 0 && resolutionsToSave[i].height != 0 &&
                resolutionsToSave[i].resName != "")
            {
        ...
        for (int i = 0; i < resolutions.Count; i++)
        {
            EditorPrefs.SetString("StoreScreenshotResolution_" + i,
                resolutions[i].resName + separator + resolutions[i].width + separator + resolutions[i].height +
                separator + resolutions[i].active);
        }
    ...
    void OnGUI()
    {
        ...
        EditorGUILayout.LabelField(
            "By pressing \"Store Screenshot > Save\" (Ctrl/Cmd+Alt+S) while in Play Mode, the screenshot will be saved for every \"Active\" resolution you have here.");
        ...
        for (int i = 0; i < resArray.Length; i++)
        {
            ...
            resArray[i].toDelete =
                EditorGUILayout.Toggle(resArray[i].toDelete, centeredStyle, GUILayout.Width(screenWidth * 0.2f));
        ...
        for (int i = 0; i < resSaveArray.Length; i++)
        {
            ...
            resSaveArray[i].resName =
                EditorGUILayout.TextField(resSaveArray[i].resName, GUILayout.Width(screenWidth * 0.2f));
            resSaveArray[i].width =
                EditorGUILayout.IntField(resSaveArray[i].width, GUILayout.Width(screenWidth * 0.2f));
            resSaveArray[i].height =
                EditorGUILayout.IntField(resSaveArray[i].height, GUILayout.Width(screenWidth * 0.2f));
        ...
        if (GUILayout.Button("Capture Screenshot", GUILayout.Height(30)))
        {
            ...
            {
                Debug.LogWarning("You need to be in Play Mode to capture a screenshot!");
            }
        }
        ...
        {
            string filePath = "Assets/InfoGame/";
